...
/*   Updated: 2023/12/08 11:43:21 by heda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_doors(int x, int y, t_data *gameinfo)
{
	int	doorv;
	int	doorh;
	...
	doorv = check_door_vertical(gameinfo, x, y);
	doorh = check_door_horizontal(gameinfo, x, y);
	if (doorh + doorv != 1)
	{
		gameinfo->map_dup = ft_db_free(gameinfo->map_dup);
...
void	ft_check_map(char **map, int height, t_data *game)
{
	...
	while (game->map_dup[i])
	{
		...
		!ft_strchr(PLAYER, game->map_dup[i][j]) && game->map_dup[i][j] != 'D')
			j++;
		if (game->map_dup[i][j] == 'D')
			j = check_doors(j, i, game);
		else
